"""OpenDisplay firmware protocol implementation."""
import logging
...
def _format_config_summary(config: GlobalConfig, mac_address: str) -> str:
    """Format OpenDisplay configuration as human-readable debug output.

    Args:
        config: Parsed OpenDisplay device configuration
        mac_address: Device MAC address for reference

    Returns:
        Multi-line formatted configuration summary
    """
    lines = [f"\nOpenDisplay Configuration for {mac_address}:"]
...
class OpenDisplayProtocol(BLEProtocol):
    """OpenDisplay firmware protocol implementation.

    Supports the new OpenDisplay BLE firmware protocol with:
    ...
    def __init__(self):
        """Initialize OpenDisplay protocol."""
        self._last_config: GlobalConfig | None = None
        self._last_fw_version: dict | None = None

    @property
    def manufacturer_id(self) -> int:
        """Bluetooth manufacturer ID for OpenDisplay firmware."""
        return 0x2446  # 9286 decimal

    @property
    def service_uuid(self) -> str:
        """BLE GATT service UUID for OpenDisplay firmware."""
    ...
    async def interrogate_device(self, connection: "BLEConnection") -> DeviceCapabilities:
        ...
        The OpenDisplay firmware sends config data in chunks:
        ...
        _LOGGER.debug("OpenDisplay device interrogation for %s", connection.mac_address)

        # Read first chunk
        response = await connection.write_command_with_response(CMD_READ_CONFIG)

        _LOGGER.debug(
            "OpenDisplay config response for %s: received %d bytes",
        ...
        _LOGGER.debug(
            "OpenDisplay config first 20 bytes: %s",
        ...
        # Strip OpenDisplay config header: [length:2][version:1]
        ...
        _LOGGER.debug(
            "OpenDisplay config header: length=%d bytes, version=%d",
            config_length,
            config_version,
        )
        ...
        # Parse complete TLV config (OpenDisplay format: [packet_number:1][packet_id:1][fixed_data])
        try:
            full_config = parse_tlv_config(bytes(packet_data))
        except ConfigValidationError as e:
            _LOGGER.error("Failed to parse OpenDisplay config for %s: %s", connection.mac_address, e)
            raise
        ...
        _LOGGER.debug(
            "OpenDisplay device %s config: %d displays, %d LEDs, %d sensors",
            connection.mac_address,
    ...
    async def read_firmware_version(self, connection: "BLEConnection") -> dict:
        ...
        _LOGGER.debug(
            "OpenDisplay firmware version for %s: %s (sha=%s)",
            connection.mac_address,
            version_str,
