...
class BLEDeviceMetadata:
    ...
    to device capabilities, handling both ATC and OpenDisplay metadata formats.
    ...
    def width(self) -> int:
        ...
        if self._is_open_display:
            displays = self._metadata["open_display_config"].get("displays", [])
    ...
    def hw_type(self) -> int:
        ...
        if self._is_open_display:
            displays = self._metadata["open_display_config"].get("displays", [])
            return displays[0].get("open_display_tagtype", 0) if displays else 0
    ...
    def power_mode(self) -> int:
        ...
        """
        if self._is_open_display:
            power = self._metadata["open_display_config"].get("power")
            if power:
    ...
    def color_scheme(self) -> ColorScheme:
        ...
        OpenDisplay: Reads from display config device_metadata["open_display_config"]["displays"][0]["color_scheme"]
        """
        if self._is_open_display:
            displays = self._metadata["open_display_config"].get("displays", [])
